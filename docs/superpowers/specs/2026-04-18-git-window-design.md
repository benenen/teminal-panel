...
> **设计原型**：查看 `docs/superpowers/specs/git-window-ui-prototype.html` 获取完整的交互式设计原型

### 设计风格

**现代技术美学**：结合专业工具的严肃感和现代设计的精致感，创造一个既美观又高效的 Git 可视化界面。
...
**边框和分隔：**
- 微妙边框：`rgba(255, 255, 255, 0.06)`
- 中等边框：`rgba(255, 255, 255, 0.1)`
- 强调边框：`rgba(88, 166, 255, 0.3)`
...
**Diff 视图颜色：**
- 新增行背景：`rgba(63, 185, 80, 0.1)`
- 删除行背景：`rgba(248, 81, 73, 0.1)`
- 行号：`#6e7681`
...
**UI 文本：**
- 字体家族：Outfit（优雅的无衬线字体）
- 备用字体：系统默认字体栈

**代码和路径：**
- 字体家族：JetBrains Mono（专业的等宽字体）
- 用于：文件路径、commit hash、diff 内容

### 图标

使用 Bootstrap Icons 或 Unicode emoji：
- 文件状态：
  - 新增：`A` 在绿色圆角方块中
  - 修改：`M` 在橙色圆角方块中
  - 删除：`D` 在红色圆角方块中
...
- 圆角：6px
- 悬停：背景色变为 `#1a1f28`
- 选中：背景色 `#1a1f28` + 蓝色边框 + 左侧彩色指示器

**提交记录行：**
- 内边距：12px 0
- 提交节点：12px 直径，带 2px 边框和外发光效果
- 提交信息：14px 字号，500 字重
- 元信息：12px 字号，次级文本颜色
